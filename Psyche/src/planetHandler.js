 ...
 * - orbitSpeed: Speed of the planet's orbit
 * - rotationSpeed: Speed of the planet's rotation
 * - eccentricity: The orbital eccentricity for elliptical orbits
 * - inclination: The orbital plane's tilt in degrees
 * - scene: The 3D scene where the planet will be rendered
 * - model: The 3D model of the planet
 * - loader: GLTFLoader instance for loading 3D models
 * - elapsedTime: Accumulated time used for orbital calculations
 */
export class PlanetHandler {
  constructor(name, modelPath, size, orbitRadius, orbitSpeed, rotationSpeed, eccentricity, inclination, scene) {
    this.name = name;
    this.size = size;
    this.orbitRadius = orbitRadius * 1.22;
    this.orbitSpeed = orbitSpeed;
  ...
  loadModel(path) {
    this.loader.load(
      ...
      (gltf) => {
        this.model = gltf.scene;
        this.model.scale.set(this.size, this.size, this.size);
        this.model.position.set(this.orbitRadius * (1 - this.eccentricity), 0, 0);
        this.scene.add(this.model);
        if (this.onLoad) {
          this.onLoad();
        }
  ...
  update(rotationSpeed, orbitSpeed, deltaTime) {
    if (this.model) {
      ...
      const semiMajorAxis = this.orbitRadius;
      const semiMinorAxis = semiMajorAxis * Math.sqrt(1 - this.eccentricity ** 2);

      const angle = -orbitSpeed * this.elapsedTime;
      const x = semiMajorAxis * Math.cos(angle);
      const z = semiMinorAxis * Math.sin(angle);

      const inclinationRadians = (this.inclination * Math.PI) / 180;

      this.model.position.x = x;
      this.model.position.y = z * Math.sin(inclinationRadians);
      this.model.position.z = z * Math.cos(inclinationRadians);
    }
  }
}
